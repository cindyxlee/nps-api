...
function formatQueryParams(params) {
    const queryItems = Object.keys(params).map(key => `${[encodeURICompent(key)]}=${encodeURIComponent(params[keys])}`);
...
function displayResults(responseJson, maxResults) {
    ...
    for (let i=0; i < responseJson.data.length & i < maxResults; i++) {
        $('.results-list').appened(`<li><h3><a href="${responseJson.data[i].url}">$${responseJson.data[i].fullName}</a></h3>
...
function watchForm() {
    $('.js-form').on('submit', function() {
        ...
        const apiKey = 'Egfe2kFMXlEJuW953OZuBjn8zbCeZ85YDfjkITTX';
